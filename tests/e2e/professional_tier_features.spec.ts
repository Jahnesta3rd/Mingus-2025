...
async function addAllMocks(p: Page) {
  await p.route('**/api/auth/login', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({ success: true, user_id: 3, email: JASMINE.email, name: 'Jasmine', message: 'Login successful' }),
    });
  });
  ...
  await p.route('**/api/auth/verify**', async (route) => {
    await route.fulfill({
      status: 200, contentType: 'application/json',
      body: JSON.stringify({ valid: true, user: { email: JASMINE.email, tier: 'professional', firstName: 'Jasmine' } }),
    });
  });
