...
async function addAllMocks(p: Page) {
  ...
  await p.route('**/api/auth/login', async (route) => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({ success: true, user_id: 2, email: MARCUS.email, name: 'Marcus', message: 'Login successful' }),
    });
  });
  ...
  await p.route('**/api/auth/verify**', async (route) => {
    await route.fulfill({
      status: 200, contentType: 'application/json',
      body: JSON.stringify({ valid: true, user: { email: MARCUS.email, tier: 'mid_tier', firstName: 'Marcus' } }),
    });
  });
