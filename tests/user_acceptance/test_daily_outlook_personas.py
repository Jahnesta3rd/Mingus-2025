...
# Set up Python path after imports (avoid backup-directory import conflicts)
...
class TestMayaPersona:
    ...
    def test_maya_daily_outlook_generation(self, client, app, maya_user, maya_relationship_status):
        ...
        with app.app_context():
            with patch(
                "tests.api.test_daily_outlook_api.get_current_user_id",
                return_value=maya_user.id,
            ):
                with patch(
                    "tests.api.test_daily_outlook_api.check_user_tier_access",
                    return_value=True,
                ):
    ...
    def test_maya_tier_restrictions(self, client, app, maya_user):
        ...
        with app.app_context():
            with patch(
                "backend.api.daily_outlook_api.get_current_user_id",
                return_value=maya_user.id,
            ):
                # Maya has budget tier access
                with patch(
                    "backend.api.daily_outlook_api.check_user_tier_access",
                    return_value=True,
                ):
                    response = client.get("/api/daily-outlook/")
...
class TestDrWilliamsPersona:
    ...
    def test_dr_williams_daily_outlook_generation(self, client, app, dr_williams_user, dr_williams_relationship_status):
        ...
        with app.app_context():
            with patch(
                "backend.api.daily_outlook_api.get_current_user_id",
                return_value=dr_williams_user.id,
            ):
                with patch(
                    "backend.api.daily_outlook_api.check_user_tier_access",
                    return_value=True,
