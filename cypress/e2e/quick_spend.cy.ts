...
function seedAuthStorage(win: Window) {
  ...
  win.localStorage.setItem("mingus_employer_backfill_dismissed", "true");
...
/** Auth + route guards for dashboard/snapshot (LoginPage redirects to /welcome). */
function setupAuthIntercepts() {
  ...
  cy.intercept("GET", "/api/user/terms-status", {
    statusCode: 200,
    body: {
      accepted: true,
      acceptedVersion: "1.0",
  ...
  cy.intercept("GET", "/api/vibe/daily", {
    statusCode: 200,
    body: { has_vibe: false },
  });
  cy.intercept("GET", "/api/profile/setup-status", {
    statusCode: 200,
    body: {
      setupCompleted: true,
...
function setupSnapshotIntercepts() {
  cy.intercept("GET", "/api/faith-card/today", {
    statusCode: 200,
    body: {
  ...
  cy.intercept("GET", "/api/life-ready-score", {
    statusCode: 200,
    body: { has_sufficient_data: false },
  });
  cy.intercept("GET", "/api/cash-flow/enhanced-forecast/*", {
    statusCode: 200,
    body: { forecast: { daily_cashflow: [] } },
  });
  cy.intercept("GET", "/api/vibe-tracker/people", {
    statusCode: 200,
    body: { people: [] },
  });
...
describe("Quick Spend Logger", () => {
  beforeEach(() => {
    ...
    cy.visit("/dashboard/tools", { onBeforeLoad: seedAuthStorage });
    cy.wait("@authVerify");
    cy.wait("@getToday");
  });
  ...
  it("closes the sheet when backdrop is tapped", () => {
    ...
    cy.contains("How much?").should("not.exist");
  });
  ...
  it("logs a spend entry and updates the today log", () => {
    ...
    cy.get("[data-testid='quick-spend-backdrop']", { timeout: 3000 }).should(
      "not.exist"
    );
  });
  ...
  describe("Tracker #155 regression — Snapshot spending card", () => {
    it("calls /api/expenses/summary not a ghost URL", () => {
      cy.visit("/snapshot", { onBeforeLoad: seedAuthStorage });
      cy.wait("@getExpensesSummary")
        .its("response.statusCode")
        .should("eq", 200);
    });

    it("snapshot still renders after spending summary loads", () => {
      cy.visit("/snapshot", { onBeforeLoad: seedAuthStorage });
      cy.wait("@getExpensesSummary");
      cy.contains("CARD 1 OF 3").should("be.visible");
    });
  });
});
