// AssessmentFlow.tsx - TypeScript version showing how to use Supabase functions
...
type AssessmentStep = 'email' | 'assessment' | 'results'
...
const AssessmentFlow: React.FC = () => {
  ...
  const handleQuestionResponse = async (questionId: string, value: string, points: number) => {
  ...
  const getUserSegment = (score: number): string => {
  ...
  const getProductTier = (score: number): string => {
          ...
          <div className="space-y-4">
            ...
            {questions[currentQuestion]?.question_type === 'radio' && (
              <div className="space-y-2">
                {JSON.parse(questions[currentQuestion].options).map((option: any, index: number) => (
                  <button
                    key={option.value}
