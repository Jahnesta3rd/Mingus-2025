...
try:
    ...
    # Try importing model symbols; if this fails we will run in shim mode
    try:
...
class PlaidIntegrationService:
    ...
    def _setup_plaid_client(self):
        ...
        if not HAVE_PLAID:
            # Shim: skip real client setup
            self.api_client = None
            self.link_token_api = None
            self.accounts_api = None
            self.transactions_api = None
            self.identity_api = None
            self.institutions_api = None
            self.item_api = None
            logger.warning("Plaid SDK not available; using shim implementation")
            return
        try:
    ...
    def create_link_token(self, user_id: str, products: List[PlaidProduct] = None) -> LinkTokenResult:
        ...
        try:
            if not HAVE_PLAID:
                # Shim: return deterministic token
                return LinkTokenResult(
                    link_token=f"link-token-{user_id}",
            ...
            if not HAVE_PLAID:
                ...
                if connection:
                    ...
                    connection = PlaidConnection(
                        ...
                        products=[p.value for p in ([])],
                        webhook_url=getattr(getattr(self.config, 'webhook', None), 'webhook_url', ''),
                        is_active=True,
                    )
