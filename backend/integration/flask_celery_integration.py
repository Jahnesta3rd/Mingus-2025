...
def execute_communication_task(task_name: str, payload: Optional[Dict[str, Any]] = None, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Lightweight execution stub for communication tasks.

    In testing, we avoid dispatching real Celery tasks. This stub logs the
    request and returns a predictable result structure used by tests.
    ...
    Returns:
        Dict with status and echo of inputs.
    """
    try:
        logger.info(f"execute_communication_task called: task_name={task_name}, options={options}")
        result: Dict[str, Any] = {
            "status": "queued",
            "task_name": task_name,
            "payload": payload or {},
        ...
        logger.error(f"execute_communication_task failed: {exc}")
        return {"status": "error", "error": str(exc), "task_name": task_name}


def execute_analytics_task(event_name: str, properties: Optional[Dict[str, Any]] = None, options: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    """
    Lightweight execution stub for analytics tasks.

    Mirrors the shape of a queued analytics task result without touching Celery.
    """
    try:
        logger.info(f"execute_analytics_task called: event_name={event_name}, options={options}")
        result: Dict[str, Any] = {
            "status": "queued",
        ...
        logger.error(f"execute_analytics_task failed: {exc}")
        return {"status": "error", "error": str(exc), "event_name": event_name}
