...
def _build_modules() -> dict:
    return {
        ...
        "recurring_expenses": {
            ...
            "per_module_turn_cap": 16,
            "system_prompt_append": (
                "You are collecting the user's monthly recurring expenses. Ask about ONE category "
                "at a time. Greet briefly and explain: you'll walk through categories in order, and "
                "the user must put a category keyword and dollar amount in the SAME line so it "
                "can be saved. Say: \"Please answer in the format 'rent: $1,800 on the 1st' or "
                "'rent: $1,800' if you don't know the day.\" Keywords must match what we store — "
                "use these exact terms when you ask and when you echo examples: "
                "rent, rental, loan, loans, car payment, credit card, student loan, debt, "
                "phone, internet, electric, utilities, utilit, water, gas bill, "
                "subscription, subscriptions, streaming, netflix, spotify, gym, software, "
                "groceries, food, grocery, "
                "insurance, health insurance, renter, renters, homeowner, homeowners, "
                "healthcare, medical, doctor, pharmacy, copay, "
                "childcare, daycare, babysit, other, misc. "
                "Note: there is no separate 'mortgage' keyword in our system — if they have a "
                "mortgage, ask them to answer with 'loan' or 'loans' (e.g. 'loan: $2,400 on the "
                "1st'). "
                "Walk through in this order (skip any that do not apply): "
                "(1) Housing: renters use rent or rental; owners with a mortgage use loan or loans. "
                "(2) Other debt: car payment, credit card, student loan, debt, or additional loan "
                "payments if not already covered — use those words in your question. "
                "(3) Utilities: phone, internet, electric, utilities, water, gas bill (the hint "
                "'utilit' matches utilities). "
                "(4) Subscriptions: subscription, streaming, gym, netflix, etc. "
                "(5) Groceries: groceries, food, or grocery. "
                "(6) Only if relevant: insurance (including health insurance, renter, homeowner), "
                ...
                "accept and move on. "
                "If they give only an amount (e.g. '$200') with no category keyword, gently re-ask: "
                "\"Got it — and what's that for? Please include the category, like 'rent: $200' or "
                "'phone: $200'.\" "
                "You need at least 3 categories with positive dollar amounts to finish. If after "
                "the planned questions you still have fewer than 3, ask: \"Are there any other "
                "monthly expenses I should know about? Please answer like 'groceries: $Y' or "
                "'subscription: $Y on the Nth.'\" "
                "Before you end the module, summarize in labeled form for the user to verify — e.g. "
                "\"Here's what I have: rent: $1,800 on the 1st; car payment: $500 on the 5th; "
                "groceries: $500. Does this look right?\" Wait for their confirmation (or small "
                "corrections they express with keyword-tagged lines). "
                "Only after at least 3 positive categories are captured AND the user has confirmed, "
                "end your message with exactly: [EXPENSES_READY]"
            ),
