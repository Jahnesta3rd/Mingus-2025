...
import { ROOF_CHECK_QUESTIONS } from "./roofCheckQuestions";

type RoofCheckQuizPanelProps = {
  onComplete: (answers: Record<string, number>) => void;
  onError: (msg: string | null) => void;
  busy?: boolean;
};

export function RoofCheckQuizPanel({ onComplete, onError, busy = false }: RoofCheckQuizPanelProps) {
  const [started, setStarted] = useState(false);
  const [qIndex, setQIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, number>>({});

  const totalQs = ROOF_CHECK_QUESTIONS.length;
  const currentQ = ROOF_CHECK_QUESTIONS[qIndex];
    ...
    return (
      <div className="space-y-6 text-center">
        <p className="text-sm leading-relaxed text-[#9a8f7e]">
          12 questions. Your answers estimate housing-related wealth-building potential — illustrative only, not
