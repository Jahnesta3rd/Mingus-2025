...
import { BODY_CHECK_QUESTIONS } from "./bodyCheckQuestions";

type BodyCheckQuizPanelProps = {
  onComplete: (answers: Record<string, number>) => void;
  onError: (msg: string | null) => void;
  busy?: boolean;
};

export function BodyCheckQuizPanel({ onComplete, onError, busy = false }: BodyCheckQuizPanelProps) {
  const [started, setStarted] = useState(false);
  const [qIndex, setQIndex] = useState(0);
  const [answers, setAnswers] = useState<Record<string, number>>({});

  const totalQs = BODY_CHECK_QUESTIONS.length;
  const currentQ = BODY_CHECK_QUESTIONS[qIndex];
    ...
    return (
      <div className="space-y-6 text-center">
        <p className="text-sm leading-relaxed text-[#9a8f7e]">
          15 questions. Your answers help estimate health-related costs and productivity impact — nothing here is
          medical advice.
        </p>
        <button
          type="button"
