...
export default function PreLaunchTracker() {
  ...
  const toggleStatus = (id: string) => {
    setItems(prev => prev.map(item => {
      ...
      const cycle: Record<string, "open" | "in-progress" | "fixed"> = { open: "in-progress", "in-progress": "fixed", fixed: "open" };
      ...
      <div style={{ maxWidth: 780, margin: "0 auto" }}>
        <div style={{ marginBottom: 28 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
            {[
              { key: "all", label: `All  ${counts.total}`, color: "#94A3B8", activeBg: "#1E293B" },
              { key: "critical", label: `🔴  ${counts.critical} Critical`, color: "#FF4444", activeBg: "#FF444420" },
              { key: "recommended", label: `🟡  ${counts.recommended} Recommended`, color: "#F59E0B", activeBg: "#F59E0B20" },
              { key: "polish", label: `🟢  ${counts.polish} Polish`, color: "#22C55E", activeBg: "#22C55E20" },
              { key: "fixed", label: `✓  ${counts.fixed} Fixed`, color: "#34D399", activeBg: "#34D39920" },
            ].map(f => (
