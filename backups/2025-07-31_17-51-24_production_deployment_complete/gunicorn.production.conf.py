"""
Production Gunicorn Configuration for MINGUS Application
Optimized for high-performance Flask deployment with proper worker management
"""
...
workers = int(os.getenv('GUNICORN_WORKERS', max(4, multiprocessing.cpu_count() * 2 + 1)))
worker_class = os.getenv('GUNICORN_WORKER_CLASS', 'gevent')
worker_connections = int(os.getenv('GUNICORN_WORKER_CONNECTIONS', 1000))

# Worker lifecycle - production optimized
max_requests = int(os.getenv('GUNICORN_MAX_REQUESTS', 1000))
max_requests_jitter = int(os.getenv('GUNICORN_MAX_REQUESTS_JITTER', 100))
worker_tmp_dir = os.getenv('GUNICORN_WORKER_TMP_DIR', '/dev/shm')

# Worker timeout settings - production values
timeout = int(os.getenv('GUNICORN_TIMEOUT', 180))  # 3 minutes for complex operations
...
def when_ready(server):
    ...
    server.log.info(f"MINGUS Production Server Ready - Spawning {workers} workers")
...
def on_starting(server):
    ...
    server.log.info("Starting MINGUS Production Gunicorn Server")
    server.log.info(f"Environment: {os.getenv('FLASK_ENV', 'production')}")

def on_reload(server):
    """Called to reload the server"""
    server.log.info("Reloading MINGUS Production Server")

def worker_int(worker):
    """Called just after a worker has been initialized"""
    worker.log.info(f"Worker {worker.pid} initialized for MINGUS")
...
def post_worker_init(worker):
    ...
    worker.log.info(f"Worker {worker.pid} initialized MINGUS application")

def worker_abort(worker):
    """Called when a worker received SIGABRT signal"""
    worker.log.error(f"Worker {worker.pid} received SIGABRT - Critical error")
...
def on_exit(server):
    ...
    server.log.info("MINGUS Production Server shutting down")
...
logger.info("=" * 60)
logger.info("MINGUS PRODUCTION GUNICORN CONFIGURATION")
logger.info("=" * 60)
...
logger.info(f"SSL Enabled: {bool(keyfile and certfile)}")
logger.info("=" * 60) 