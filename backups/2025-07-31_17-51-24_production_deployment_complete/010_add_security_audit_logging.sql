-- 010_add_security_audit_logging.sql
-- Migration: Add security audit logging and enhanced security features
...
ALTER TABLE phone_verification ADD COLUMN captcha_verified BOOLEAN DEFAULT FALSE; -- CAPTCHA verification status
...
-- Create view for high-risk activities
CREATE VIEW IF NOT EXISTS high_risk_activities AS
SELECT 
    user_id,
    ip_address,
    phone_number,
    ...
    risk_score,
    created_at,
    COUNT(*) OVER (PARTITION BY ip_address ORDER BY created_at ROWS BETWEEN 0 AND 23) as events_last_24h
...
UPDATE phone_verification SET captcha_verified = FALSE WHERE captcha_verified IS NULL;
